...
def _tidy_lines(text):
    """Построчная чистка шва. Отступы и строки таблиц не трогаем: там пробел значим."""
    out = []
...
#: Зависимый хвост после фразы: «думай пошагово О конкурентных сценариях». Вырезать
#: такую фразу нельзя — дополнение останется без своего глагола и пришьётся к чужому.
_COT_TAIL = re.compile(r"\s*(?:о|об|обо|про|about|on|when|при)\b", re.I)
_COT = re.compile(
...
def _strip_cot(text):
    ...
    Третья: точка в голове шаблона съедала границу предложения. «Реализуй кэш в
    @src/cache.py. Думай пошагово о конкурентных сценариях» превращалось в «Реализуй
    кэш в @src/cache.py о конкурентных сценариях» — дополнение вырезанной фразы
    пришивалось к чужому глаголу. Теперь точку не трогаем, а фразу с зависимым
    хвостом не режем вовсе: находка всё равно показана, решение за человеком.
    """
    out, prev = [], 0
    for m in _COT.finditer(text):
        if _COT_TAIL.match(text, m.end()):
            continue                       # у фразы есть зависимое — не трогаем
...
def _strip_show_thinking(text):
    ...
    Отдельный нож, а не переиспользование `_strip_cot`: у правил разный смысл.
    Раньше правило `fable5-no-show-thinking` стояло с `action: remove`, но ветки
    в переписчике под него не было — отчёт печатал «[-] Убрать», а текст оставался
    прежним. Обещать правку и не делать её хуже, чем не обещать.
    """
    from .analyze import _SHOW_THINKING

    out, prev = [], 0
    for m in _SHOW_THINKING.finditer(text):
        head = re.search(r"(?:[,;]?\s*(?:\bи\s+)?(?:обязательно\s+)?)$", text[:m.start()])
...
def _dedupe_sentences(text):
    ...
    for line in text.split("\n"):
        ...
        for p in parts:
            ...
            if len(key) > 12 and key == seen.get("last"):
                continue
            if len(key) > 12:
                seen["last"] = key
            kept.append(p)
