...
from .analyze import CODE_SPAN as _CODE_SPAN, DEPENDENT_TAIL as _COT_TAIL, task_shape
...
def cut_phrase(text, pattern, *, tail=_COT_TAIL, head=_CUT_HEAD):
    """ЕДИНСТВЕННЫЙ способ вырезать фразу из промпта.

    Хвост-гард встроен сюда, а не вызывается по памяти в каждом ноже. Иначе
    получается ровно то, что и получилось: гард поставили в `_strip_cot`, а
    новый нож `_strip_show_thinking`, написанный в том же коммите РАДИ этой
    болезни, ушёл без него — и «Распиши рассуждения о рисках миграции и почини
    @db.py» превращалось в « о рисках миграции и почини @db.py».

    Правило: фразу с зависимым хвостом («… О рисках», «… ABOUT choices») не
    режем вовсе. Дополнение без своего глагола пришивается к чужому и меняет
    задание. Находка при этом всё равно показана — решает человек.
    """
    out, prev = [], 0
    for m in pattern.finditer(text):
        if tail and tail.match(text, m.end()):
            continue
        start = m.start()
        if head:
...
def _strip_vague(text):
    text2 = re.sub(
        # «можешь\b» без гарда превращало разрешение в запрет: «Можешь не трогать
        # прод» → «Не трогать прод». Отрицание после «можешь» — это смысл, а не
        # вежливость.
        r"(?i)^\s*(пожалуйста,?\s*)?(не мог(ли)? бы (ты|вы)|мог бы ты|можешь ли ты|"
        r"можешь\b(?!\s+не\b)|could you\s+please|could you|can you|would you)[,:]?\s*",
        "",
        text,
    )
...
_REPORT_ONLY_WHOLE = re.compile(
    r"(?i)только\s+(?:самое\s+)?(?:важное|критичное|значимое|серьёзное|"
