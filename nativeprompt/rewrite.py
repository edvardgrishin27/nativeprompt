...
#: стоит одно из: конец текста, конец предложения, перевод строки, запятая или
#: точка с запятой, союз. Всё прочее — вероятное зависимое слово, и фраза
#: остаётся на месте.
_SAFE_TAIL = re.compile(r"\s*(?:$|[.!?;,\n]|\b(?:и|а|но|затем|потом|and|then|but)\b)", re.I)
...
_NEGATION = re.compile(r"(?i)\b(?:не|don'?t|do not|never)\b")
