...
_RU_ENDING = r"(?:[аеиоуыюяьйё]{0,2})\b"
_CODE_NOUN_STRICT = ("функци", "метод", "класс", "модул", "скрипт", "тест", "схем",
                     "баг", "ветк", "апи", "сборк", "билд")
#: Длинные корни однозначны: слова, которые с них начинаются, и так про код.
_CODE_NOUN_LOOSE = ("эндпоинт", "endpoint", "роут", "route", "компонент", "миграци",
                    "репозитор", "коммит", "парсер", "валидац", "хендлер", "handler",
                    "конфиг", "config", "деплой", "ошибк", "query", "function",
...
def task_shape(prompt):
    ...
    if len(prompt.strip()) <= 120 and _SHAPE_TRIVIAL.search(low):
        ...
        if not re.search(r"[:;]|\bи\b.*\bи\b", prompt):
            return "trivial"
