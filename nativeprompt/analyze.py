...
_MUTATING_VERB = re.compile(
    ...
    r"реализ|внедри|обнови|мигрир|\bfix\b|refactor|implement|optimi[sz]e|migrate)",
    re.I,
)
...
def _c_verification_demand(p, low, t):
    ...
    """
    return bool(bare_verification_spans(p))


#: Хвост фразы «убедись, что …» до конца предложения — вырезать надо его целиком,
#: иначе остаётся обрубок вроде «В самом конце ещё раз».
_PHRASE_TAIL = re.compile(r"[^.!?\n]*")
#: Присоединённое слева «и», «а также», «обязательно» — часть той же вставки.
_PHRASE_HEAD = re.compile(r"(?:[,;]?\s*(?:и|а также)\s+)?(?:обязательно\s+)?$", re.I)


def bare_verification_spans(p):
    """Спаны ГОЛЫХ просьб перепроверить себя — то, что можно безопасно вырезать.

    Один источник правды для детектора и для переписчика. Раньше их было два:
    детектор научился щадить фразу с требованием к результату, а нож в `rewrite`
    остался глобальной регуляркой и продолжал резать всё подряд. На смешанном
    промпте — где рядом и защищённая «убедись, что признак всплывает», и голая
    «в самом конце перепроверь себя» — детектор срабатывал по второй, а вырезались
    обе, и условие «Если не всплывёт» снова повисало в пустоте.
    """
    spans = []
    for m in _VERIFY_PHRASE.finditer(p):
        ...
        end += _PHRASE_TAIL.match(p, end).end() - end     # добрать до конца предложения
        head = _PHRASE_HEAD.search(p, 0, start)
        if head:
            start = head.start()
        # Если во фразе-носителе не остаётся собственного дела — убрать предложение
        # целиком. Иначе от «В самом конце перепроверь себя ещё раз.» остаётся
        # обрубок «В самом конце.», который выглядит как потерянный текст.
        sent_start = max((p.rfind(ch, 0, start) for ch in ".!?\n"), default=-1) + 1
        prefix = p[sent_start:start]
        if not _CODING_VERB.search(prefix) and not _DELIVERABLE_VERB.search(prefix):
            start = sent_start
...
_TRIVIAL_STRONG = re.compile(
    r"(переименуй|опечат|\btypo\b|\brename\b|добавь лог|add a log|один символ)",
    re.I,
)
