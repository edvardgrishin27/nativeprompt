...
#: Английским корням нужна ПРАВАЯ граница: без неё «test» находился внутри
#: «testimony», «class» внутри «classical», «script» внутри «scripture»,
#: «api» внутри «apiary», «build» внутри «building».
_CODE_NOUN_EN = ("endpoint", "route", "handler", "config", "query", "function",
                 "method", "class", "module", "script", "component", "migration",
                 "parser", "api", "bug", "test", "build")
...
_CODE_NOUN = re.compile(
    ...
    + "|"
    + "|".join(r + r"(?:s|es|ing)?\b" for r in _CODE_NOUN_EN)
    + r")",
    re.I,
)
...
_RESEARCH_STRONG = re.compile(
    r"(обзор рынка|анализ рынка|сравни\w* цен|сравни\w* тариф|конкурент\w*|"
    r"market (overview|research)|competitive|подбер\w* источник|дайджест)",
    re.I,
)


def _is_research(low, prompt=None):
    """Разведка — только пока в задаче нет предмета из кода.
...
def bare_verification_spans(p):
    ...
    for m in _VERIFY_PHRASE.finditer(p):
        ...
        protected = bool(_CONSEQUENCE.search(tail_text))
...
#: Блок кода или инлайн-код: для детектора это ДАННЫЕ, а не инструкции.
...
def task_shape(prompt):
    ...
    if len(prompt.strip()) <= 120 and _SHAPE_TRIVIAL.search(low):
        # Двоеточие или перечисление означают, что за «подправь» стоит спецификация:
        # «Подправь расчёт комиссии: округление вверх, потолок 500 ₽» — правка
        # денежной логики, а не опечатка. Такое гасить обвязкой нельзя.
        # Двоеточие само по себе тривиальности не лишает: «Исправь опечатку:
        # "превет" → "привет"» — это уточнение, а не спецификация. Лишает
        # ПЕРЕЧИСЛЕНИЕ требований после него: «округление вверх, потолок 500».
        spec = re.search(r"[:;]\s*[^,\n]+,\s*[^,\n]+", prompt) or re.search(r"\bи\b.*\bи\b", prompt)
        if not spec:
            return "trivial"
