...
# ── седьмой круг ────────────────────────────────────────────────────
def test_граница_слова_перед_глаголом_вырезания():
    """Худшее за все круги: «дУМАЙ» находилось внутри «обДУМАЙ».

    «Обдумай пошагово» превращалось в «Об.», "Rethink step by step" в "Re." —
    задача уничтожалась целиком. Тот же класс, что чинился на списке
    ...
    for src in ("Обдумай пошагово", "Продумай пошагово задачу", "Rethink step by step"):
        # Проверяем ОБЕ половины по отдельности: детектор и нож прикрывают друг
        # друга, и мутация в одной из них через `build_report` не видна.
        assert not _FORCED_COT.search(src), "детектор: %r" % src
        assert _strip_cot(src) == src, "нож: %r -> %r" % (src, _strip_cot(src))
        out = build_report(src, "gpt-5.6")["improved"].split("\n")[0]
        assert src in out, "%r -> %r" % (src, out)
    # А настоящее указание модели по-прежнему убирается.
    assert _FORCED_COT.search("Напиши парсер, думай пошагово")
    assert "думай" not in build_report("Напиши парсер, думай пошагово", "gpt-5.6")["improved"]


def test_отрицание_перед_только_важным_не_переворачивается():
    """«Покажи не только важное, но и мелкие» — автор уже просит всё."""
    from nativeprompt.rewrite import _report_all

    src = "Покажи не только важное, но и мелкие замечания"
    assert _report_all(src) == src
    assert "всё" in _report_all("Покажи только самое важное.")


def test_вырезание_в_начале_текста_не_оставляет_разделитель():
    """«Думай пошагово, почини баг» давало «, почини баг»."""
    from nativeprompt.rewrite import _strip_cot

    assert _strip_cot("Думай пошагово, почини баг") == "почини баг"
    assert _strip_cot("Думай пошагово. Почини баг") == "Почини баг"


def test_данные_защищены_в_кавычках_любого_вида():
    """Защита прямых кавычек была белым списком из одного вида: ёлочки, лапки и
    одинарные по-прежнему портились, а в русском ёлочки — стандарт."""
    from nativeprompt.explain import build_report

    for кавычки in ('"a ; b"', "«a ; b»", "„a ; b“", "'a ; b'"):
        src = "ОБЯЗАТЕЛЬНО замени %s на что-то в @f.py" % кавычки
        out = build_report(src, "claude-opus-5")["improved"]
        assert кавычки in out, out


def test_дедуп_ловит_повтор_при_любой_разбивке():
    """Три токенизации давали «[-] Убрать повторы» при нулевой правке."""
    from nativeprompt.rewrite import _dedupe_sentences

    assert _dedupe_sentences("Сделай бэкап базы данных. Сделай бэкап базы данных") == \
